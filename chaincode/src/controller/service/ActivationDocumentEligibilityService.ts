...
export class ActivationDocumentEligibilityService {
    public static checkEligibilityStatus(
        ...
        if (!activationDocumentObj.eligibilityStatus
            ...
            const pattern = activationDocumentObj.messageType
    ...
    public static statusInternationalValue(statusValue: string): string {
        let newStatus = EligibilityStatusType.EligibilityERROR;

        if (!statusValue) {
            newStatus = EligibilityStatusType.EligibilityPending;
        } else {
            statusValue = statusValue.toLowerCase();
            if (statusValue === EligibilityStatusType.EligibilityAccepted
                || statusValue === EligibilityStatusType.FREligibilityAccepted) {
                newStatus = EligibilityStatusType.EligibilityAccepted;
            }
            if (statusValue === EligibilityStatusType.EligibilityRefused
                || statusValue === EligibilityStatusType.FREligibilityRefused) {
                newStatus = EligibilityStatusType.EligibilityRefused;
            }
            if (statusValue === EligibilityStatusType.EligibilityPending
                || statusValue === EligibilityStatusType.FREligibilityPending) {
                newStatus = EligibilityStatusType.EligibilityPending;
            }
        }
    ...
    public static async outputFormatFRActivationDocument(
        ...
        activationDocument.eligibilityStatus =
            ActivationDocumentEligibilityService.statusFrenchValue(activationDocument.eligibilityStatus);
    ...
    public static async formatActivationDocuments(
        ...
        const returnedList: ActivationDocument[] = [];
