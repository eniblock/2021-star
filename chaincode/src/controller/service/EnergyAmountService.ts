...
export class EnergyAmountService {
    public static async write(
        ...
        await StarPrivateDataService.write(
            params, {id: energyObj.energyAmountMarketDocumentMrid, dataObj: energyObj, collection: target});
    ...
    public static async getQueryArrayResult(
        ...
        if (arg.collection && arg.collection.length > 0) {
            collections = await HLFServices.getCollectionsOrDefault(
                params, ParametersType.DATA_TARGET, [arg.collection]);
        } else {
        ...
        if (collections) {
            for (const collection of collections) {
                const results = await QueryStateService.getPrivateQueryArrayResult(
                    params, { query: arg.query, collection});
