...
do
    tabindex=$(echo ".["$i-1"]")
    yellowPageMrid=$(tr -dc 0-9 </dev/urandom | head -c 10 ; echo '')
    ENEDIS_ENERGYACCOUNTS_VALUE=$(echo $ENEDIS_ENERGYACCOUNTS | jq $tabindex | jq --arg value $yellowPageMrid '. + {yellowPageMrid: $value}')
...
do
    tabindex=$(echo ".["$i-1"]")
    yellowPageMrid=$(tr -dc 0-9 </dev/urandom | head -c 10 ; echo '')
    RTE_ENERGYACCOUNTS_VALUE=$(echo $RTE_ENERGYACCOUNTS | jq $tabindex | jq --arg value $yellowPageMrid '. + {yellowPageMrid: $value}')
